...
Cave = Struct.new(:id, :slist)
track = [Cave.new(0, Set.new([0]))]
cur_step = 0
tracks_num = 0
while cur_step < track.length do
  cur_cave = track[cur_step]
  if cur_cave.id == cave_indexes['start']
    tracks_num += 1
  else
    (0...num_caves).each do |i|
      if cave_connections[cur_cave.id][i] == 1 && (is_big[i] || !cur_cave.slist.include?(i))
        next_cave = Cave.new(i, cur_cave.slist)
        unless is_big[i]
